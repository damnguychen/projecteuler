...
def CheckProduct(num, num1, num2):	# check if a number is a product of two x-digit numbers
	for i in range(num2, num1-1, -1):
		if (num % i==0):
			result=int(num/i)
			if (len(str(result))==len(str(num1))):
				return True
	return False

def LargestPalindrome(num1, num2):	# find largest palindrome made from product
	MaxNum=num2*num2
	MinNum=num1*num1
	for i in range(MaxNum, MinNum-1, -1):	# num2 is the max x-digit possible, num1 is the min possible
